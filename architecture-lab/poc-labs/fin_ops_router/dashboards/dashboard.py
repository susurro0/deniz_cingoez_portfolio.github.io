...
with content_col:
    ...
    if not df.empty:
        ...
        c4.metric("Avg Input", f"{int(df['usage_input'].mean())}")
        c5.metric("Avg Output", f"{int(df['usage_output'].mean())}")
    else:
    ...
    with col_chart1:
        ...
        cost_dist = (
            df.groupby("model")["cost_estimated"]
            .sum()
            .reset_index()
        )
        st.bar_chart(
    ...
    st.dataframe(
        ...
        width="stretch",
        column_config={
            "cost_estimated": st.column_config.NumberColumn(
                "Cost", format="$%.4f"
            ),
